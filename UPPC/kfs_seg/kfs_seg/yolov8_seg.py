...
class Yolov8Node(Node):
    ...
    def __init__(self):
        ...
        rclpy.logging.set_logger_level('kfs_seg', rclpy.logging.LoggingSeverity.INFO)
        ...
        self.declare_parameter("depth_threshold", 5.0)
        ...
        self.threshold = self.get_parameter("conf_thres").get_parameter_value().double_value
        ...
        # Publishers
        self._item_dict_pub = self.create_publisher(
            YoloResult, "/yolo/result", 
            10)
        
        self._pred_pub = self.create_publisher(
            Image, "/yolo/prediction/image", 
            10)
    ...
    def calculate_yaw_from_mask(self, mask_pts):
        """
        利用 PCA 计算物体掩码的主方向 (Yaw)
        :param mask_pts: 掩码像素点坐标列表 (N, 2)
        :return: yaw 角度 (弧度), 范围 [-pi, pi]
        """
        if len(mask_pts) < 10: 
            return 0.0
        pts = mask_pts.reshape(-1, 2).astype(np.float32)
        mean, eigenvectors = cv2.PCACompute(pts, mean=None)
        axis = eigenvectors[0] # [vx, vy]
    ...
    def bg_removal(self, color_img_msg: Image, depth_img_msg: Image):
        if self.color_image_msg is not None and self.depth_image_msg is not None:
            ...
            cv_depth_image = self.cv_bridge.imgmsg_to_cv2(depth_img_msg, desired_encoding='bgr8')
    ...
    def object_segmentation(self):
        if self.enable_yolo and self.color_image_msg is not None and self.depth_image_msg is not None:
            ...
            for detection in results:
                ...
                objects_yaw = []
                res_msg = YoloResult()
                res_msg.header = self.color_image_msg.header
                ...
                for i in range(n_objects):
                    ...
                    current_mask_pts = detection.masks.xy[i]
                    yaw = self.calculate_yaw_from_mask(contours[i])
                    objects_yaw.append(yaw)


                # Item dict creation
                detection_class = detection.boxes.cls.cpu().numpy()
                
                for class_idx, median_tf, yaw_val in zip(detection_class, objects_median_center_transform, objects_yaw):
                    obj = YoloObject()
                    obj.class_name = detection.names[int(class_idx)]
                    
                    # 填充坐标 (x, y, z), yaw
                    obj.x = float(median_tf[0])
                    obj.y = float(median_tf[1])
                    obj.z = float(median_tf[2])
                    obj.yaw = float(yaw_val)
                    
                    res_msg.objects.append(obj)
                self._item_dict_pub.publish(res_msg)
                detected_names = [obj.class_name for obj in res_msg.objects]
                self.get_logger().info(f"Published objects: {detected_names}")
                self.get_logger().debug("Item dictionary succesfully created and published")
                if res_msg.objects:
                    first_obj = res_msg.objects[0]
                    self.get_logger().info(
                        f"Pub: {len(res_msg.objects)} objs. "
                        f"#{first_obj.class_name}: xyz=({first_obj.x:.2f}, {first_obj.y:.2f}, {first_obj.z:.2f}), yaw={first_obj.yaw:.2f}"
                    )
