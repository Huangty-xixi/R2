...
static uint8_t s_kfs_taken_this_station;   /* 1=本站取过KFS，换站清零，防 s_kfs_j 残留误触发同桩跳过 */
...
static app_zone2_get_kfs_rel_t s_kfs_tail_rel;
                                        ...
                                        app_zone2_get_kfs_rel_t rel)
{
    ...
    if (allow_early_after_chassis != 0U && Process_GetKFS_IsChassisForwardDone() != 0U)
    {
        ...
        s_kfs_tail_j = s_kfs_active_j;
        s_kfs_tail_rel = s_kfs_active_rel;
...
{
    if (s_kfs_tail_j == Z2_KFS_ACTIVE_J_NONE)
        return;
    ...
    if (Process_GetKFS_IsBusy() == 0U)
    {
        s_kfs_done_mask |= (uint16_t)(1U << s_kfs_tail_j);
        s_kfs_tail_j = Z2_KFS_ACTIVE_J_NONE;
        return;
    }

    Process_GetKFS(s_kfs_tail_rel);
    if (Process_GetKFS_IsBusy() == 0U)
    {
        s_kfs_done_mask |= (uint16_t)(1U << s_kfs_tail_j);
        s_kfs_tail_j = Z2_KFS_ACTIVE_J_NONE;
...
static void z2_sched_after_station_kfs_done(void)
{
    ...
    if (s_path_idx < plen)
    {
        ...
        /* 同桩跳过定位：本站取过KFS && 刚取完KFS的桩 == 下个path桩 && 需要上桩 → 直发raise */
        ...
        s_kfs_taken_this_station = 0U; /* 换站复位 */
        s_kfs_j = Z2_KFS_ACTIVE_J_NONE; /* 换站清空，防 s_kfs_j 残留误触发同桩跳过 */
        z2_exec_reset_act_flags();
        s_face_dir_step_done = 0U;
...
static void z2_sched_kfs_turn(void)
{
    ...
    if (s_kfs_j != j)
    {
        ...
        /* main_lift 提前到目标高度，与 face+recenter 并行 */
        {
    ...
    s_kfs_j = j;
    s_kfs_taken_this_station = 1U; /* 本站确已取KFS，允许同桩跳过 */
