 ...
 * @brief   R2 ����Э����� �� STM32 ��ʵ�֣��������ļ�������λ��Э�飩
 *
 * �÷�:
 *   1. rc_init(uart1_putc, HAL_GetTick)��ODOM ֡������ڲ� latest_odom�������� rc_get_latest_odom / rc_odom_is_valid
 *   2. ���� ODOM �ص���rc_init �� rc_set_odom_callback(cb)��δע��������� latest_odom
 *   3. UART RX �����⴮���հ���: rc_feed_byte(rx_data);
 *   4. ��ѭ��: rc_poll();
 */
#include "upper_pc_protocol.h"
#include "app_zone2.h" /* APP_ZONE2_RED_SIDE Ĭ�ϼ� app_init.h���������/���볡һ�£�ODOM xy ����� handle_odom */
#include "common.h"
#include <string.h>
...
static uint8_t  rx_sync = 0;  /* 0=��ͬ�� 1=�ҵ� SYNC1 2=�ҵ� SYNC2 */
...
static void handle_odom(const uint8_t *data, uint16_t len)
{
    ...
    /* ǰ�� float�������Ϊ�״��������ꣻ���� = �״� - (dx,dy) �� odom_center_offset����ǰ���������
     * ���г���Ϊ�״����ƫ�ã�����Ŀ�� (1.4, 0.4) ʱ O_radar = (1.4, 0.4) + (dx,dy)_FRONT�� */
