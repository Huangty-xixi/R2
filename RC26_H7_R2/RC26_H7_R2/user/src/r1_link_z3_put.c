...
static uint8_t r1_link_z3_put_calc_chk(uint8_t cmd_id)
{
    return (uint8_t)(R1_LINK_Z3_PUT_SYNC1 ^ cmd_id);
}
...
{
    if (frame4 == NULL || r1_link_z3_put_cmd_id_valid(cmd_id) == 0U)
    {
        return;
    }

    frame4[0] = R1_LINK_Z3_PUT_SYNC1;
    frame4[1] = cmd_id;
    frame4[2] = r1_link_z3_put_calc_chk(cmd_id);
    frame4[3] = R1_LINK_Z3_PUT_SYNC2;
}
                               ...
                               uint8_t frame4[R1_LINK_Z3_PUT_FRAME_BYTES])
{
    ...
    r1_link_z3_put_frame_pack_cmd_id(cmd->cmd_id, frame4);
}
                                    ...
                                    r1_link_z3_put_cmd_t *out)
{
    ...
    if (frame4[0] != R1_LINK_Z3_PUT_SYNC1 || frame4[3] != R1_LINK_Z3_PUT_SYNC2)
    {
        return 1U;
    }

    if (frame4[2] != r1_link_z3_put_calc_chk(frame4[1]))
    {
        return 2U;
    }

    if (r1_link_z3_put_cmd_id_valid(frame4[1]) == 0U)
    {
        return 3U;
    }
                                    ...
                                    uint8_t frame4[R1_LINK_Z3_PUT_FRAME_BYTES])
{
    ...
    if (ctx->idx < R1_LINK_Z3_PUT_FRAME_BYTES)
    {
        ctx->buf[ctx->idx] = b;
        ctx->idx++;
        ...
        {
            ctx->idx = 0U;
            if (ctx->buf[0] == R1_LINK_Z3_PUT_SYNC1 &&
                ctx->buf[R1_LINK_Z3_PUT_FRAME_BYTES - 1U] == R1_LINK_Z3_PUT_SYNC2)
            {
                (void)memcpy(frame4, ctx->buf, (size_t)R1_LINK_Z3_PUT_FRAME_BYTES);
                return 1U;
            }
            if (b == R1_LINK_Z3_PUT_SYNC1)
            {
