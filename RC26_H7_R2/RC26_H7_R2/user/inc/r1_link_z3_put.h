 ...
 * @brief R1/R2 4 字节三区放三层帧：55 + cmd_id + chk + AA（chk = SYNC1 ^ cmd_id）
 *
 * === 业务调用链 ===
 * USART10: HAL_UART_RxCpltCallback -> R1Link_OnRxByte
 *   -> r1_link_z3_put_rx_feed_byte()
 *   -> r1_link_on_z3_put_frame(frame4)
 *   -> r1_link_z3_put_frame_decode()
...
#define R1_LINK_Z3_PUT_WIRE_CMD_ID_MAX    1U
